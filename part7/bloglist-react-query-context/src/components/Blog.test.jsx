...
describe('<Blog component />', () => {
  ...
  test('show only title and author but not url and likes', () => {
    ...
    const { container } =  render(<Blog  blog={blog} currentUser={authenticatedUser}/>)

    const div = container.querySelector('.blogCard')

    expect(div).toHaveTextContent(
      'Donec ultrices semper lectus ut imperdiet'
    )
    expect(div).toHaveTextContent(
      'Odessa Crane'
    )
    expect(div).not.toHaveTextContent(
      'https://example.com/svn-ux-mvc'
    )
    expect(div).not.toHaveTextContent(
      'likes 6'
    )
  ...
  test('validate if like button is clicked twice the function is called twice', async () => {
    ...
    render(<Blog  blog={blog} currentUser={authenticatedUser} updateBlog={mockHandler}/>)
